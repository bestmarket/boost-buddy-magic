...
DROP TRIGGER IF EXISTS scripts_updated ON public.scripts;
...
CREATE UNIQUE INDEX IF NOT EXISTS posts_video_channel_idx ON public.posts (video_id, channel_id);
...
CREATE INDEX IF NOT EXISTS source_videos_source_idx ON public.source_videos (source_id, position);
...
DROP POLICY IF EXISTS "Users manage their own source videos" ON public.source_videos;
CREATE POLICY "Users manage their own source videos"
ON public.source_videos FOR ALL TO authenticated
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

DROP TRIGGER IF EXISTS source_videos_set_updated_at ON public.source_videos;
